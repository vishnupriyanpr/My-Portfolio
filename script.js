...
Note                 formData.append('message', message);
                
...
                
                if (response.ok) {
F                   showNotification('Message sent successfully! I\'ll get back to you soon.', 'success');
                    contactForm.reset();
                } else {
                    // Get error message from Web3Forms
s                   const result = await response.json();
                    throw new Error(result.message || 'Failed to send message');
                }
                
            } catch (error) {
Note                 console.error('Error sending email:', error);
                showNotification(error.message || 'Failed to send message. Please try again.', 'error');
            } finally {
                // Reset button state
      S             submitBtn.innerHTML = originalText;
...
s   const tilts = ['tilt-1','tilt-2','tilt-3',''];
...
// **--- FIX #3: Removed the extra, unmatched `});` that was here ---**
